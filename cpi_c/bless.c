...
char *corresp_rparen_addr(char *c) {
    int level = 0;
    do {
        if (*c == '(') level += 1;
        else if (*c == ')') level -= 1;
...
char *corresp_lparen_addr(char *c) {
    int level = 0;
    do {
        if (*c == '(') level += 1;
        else if (*c == ')') level -= 1;
...
static struct StringSlice str_lt_term(char *op_loc) {
    char *start, *end;
    struct StringSlice retval;

    for (end = op_loc - 1; *end == ' '; end -= 1);
    end -= 1;

    if (*end == ')') {
        start = corresp_lparen_addr(end);
    }
    else {
        for (start = end;
...
static struct StringSlice str_rt_term(char *op_loc) {
    ...
    for (start = op_loc; *start == ' '; start += 1);
    start += 1;

    if (*start == '(') {
        end = corresp_rparen_addr(start);
    }
    else {
        for (end = start;
            (*start != '+') && (*start != '-') && (*start != '*') && (*start != '/') && (*start != ' ');
